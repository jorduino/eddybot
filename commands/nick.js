...
export default {
	data: new SlashCommandBuilder()
		...
		.addUserOption(option => option.setName('user')
			.setDescription('User to set nickname')
			.setRequired(true)
	...
	async execute(interaction) {
		...
		try {
			...
			await interaction.reply({ content: `Successfully changed ${targetUser}'s nickname to ${newNickname}` });
		} catch (error) {
			console.error("Couldn't change nickname\n" + error);
			await interaction.reply({ content: 'I do not have permission to change that nickname.', flags: MessageFlags.Ephemeral });
		}
	},
};
